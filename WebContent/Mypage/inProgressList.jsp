...
<%
	List<PListVO> plist3 = (List<PListVO>)request.getAttribute("plist3");
	System.out.println("plist3: " + plist3);
	pageContext.setAttribute("plist3", plist3);
%>
...
<script>
	...
	function deletePlist(frm) {
		frm.action="mypageController?type=deletePlist"
		frm.submit();
	}
</script>
</head>
<body>
	...
	<div class="container">
	<ul class="tabs">
		<li class="tab-link" data-tab="tab-1">판매중</li>
		<li class="tab-link current" data-tab="tab-2"><a href="../Mypage/mypageController?type=inProgressList">거래진행중</a></li>
		<li class="tab-link" data-tab="tab-3"><a href="../Mypage/mypageController?type=soldList">판매완료</a></li>
	</ul>
	
	<form method="post">
	<div id="tab-2" class="tab-content current">
	<c:if test="${empty plist3 }">
	<h2 style="color:gray; text-align: center; padding: 200px;">거래진행중인 상품이 없습니다.</h2>
	</c:if>
	<c:if test="${not empty plist3 }">
		<c:forEach var="pvo" items="${plist3 }">
			<fieldset>
			<table>
				<tbody>
					<tr>
						<td rowspan="5"><img src="${pageContext.request.contextPath }/uplo/${pvo.picture01 }" width=100px height=100px alt="상품사진"></td>
						<td>${pvo.p_title }</td>
					</tr>
					<tr>
						<td>상품등록일: ${pvo.p_regdate }</td>
					</tr>
					<tr>
						<td>상품가격: ${pvo.price }원</td>
					</tr>
					<tr>
						<td>${pvo.common_name }</td>
					</tr>
					<tr>
						<td>구매자 아이디: ${pvo.purchase_id }</td>
					</tr>
				</tbody>
			</table>				
			<input type="button" value="판매완료로 변경" onclick="changeStatus(this.form)">
			<input type="hidden" value="${pvo.p_no }" name="p_no">
			<input type="hidden" value="${pvo.purchase_id }" name="purchase_id">
			<input type="button" value="삭제" onclick="deletePlist(this.form)">
			</fieldset>	
		</c:forEach>
	</c:if>
	</div>
