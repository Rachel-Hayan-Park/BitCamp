...
<%
	List<PListVO> plist2 = (List<PListVO>)request.getAttribute("plist2");
...
<script>
	...
	function changeStatus(frm) {
		frm.action="mypageController?type=changeStatusToSelling"
		frm.submit();
	}
...
<body>
	...
	<div class="container">
		<ul class="tabs">
			<li class="tab-link" data-tab="tab-1"><a href="../Mypage/mypageController?type=sellingList">판매중</a></li>
			<li class="tab-link" data-tab="tab-2"><a href="../Mypage/mypageController?type=inProgressList">거래진행중</a></li>
			<li class="tab-link current"  data-tab="tab-3"><a href="../Mypage/mypageController?type=soldList">판매완료</a></li>
		</ul>
	
		<form method="post">
		<div id="tab-3" class="tab-content current">
		<c:if test="${empty plist2 }">
			<h2 style="color:gray; text-align: center; padding: 200px;">판매완료된 상품이 없습니다.</h2>
		</c:if>
		<c:if test="${not empty plist2 }">
			<c:forEach var="pvo" items="${plist2 }">
				<fieldset>
				<table>
					<tbody>
						<tr>
							<td rowspan="3"><img src="${pageContext.request.contextPath }/uplo/${pvo.picture01 }" alt="상품사진" ></td>
